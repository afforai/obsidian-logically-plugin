<script lang="ts">
  ...
  async function loadFiles() {
    ...
    try {
      ...
      if (files.length === 0 && typeof app.vault.getFiles === "function") {
        ...
        if (allVaultFiles && allVaultFiles.length > 0) {
          ...
          if (IS_DEV_BUILD) {
            console.log("[FilePicker] Filtered to md files:", files.length);
          }
        }
      }
      ...
      if (files.length === 0 && app.vault.getRoot) {
        ...
        if (root && root.children) {
          ...
          files = walk(root);
          if (IS_DEV_BUILD) {
            console.log("[FilePicker] Walking vault root found:", files.length);
          }
        }
      }
  ...
  onMount(() => {
    if (IS_DEV_BUILD) {
      console.log("[FilePicker] onMount, app:", !!app, "vault:", !!app?.vault);
    }
            ...
            <div class="ra-folder-group">
              ...
              {#each group.files as file (file.path)}
                ...
                <button
                  ...
                  disabled={!isSelected && selectedFiles.length >= maxFiles}
                >
                  <span class="ra-check">{isSelected ? "✓" : ""}</span>
                  <svg
                  ...
                  <span class="ra-file-basename">{file.basename}</span>
                </button>
              {/each}
            </div>
          {/each}
